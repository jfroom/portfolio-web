...
function getAndroidSDKTargets(apiLevels, abis) {
    return apiLevels.map(function (level) { return 'android-' + level; })
        ...
        return targets.concat(apiLevels.map(function (level) { return 'sys-img-' + architecture + '-' + deviceType + '-' + level; }));
    }, []));
}
...
var AVDDescriptor = (function () {
    ...
    AVDDescriptor.prototype.avdName = function (version) { return this.name + '-v' + version + '-wd-manager'; };
...
function sequentialForEach(array, func) {
    var ret = q(null);
    array.forEach(function (x) { ret = ret.then(function () { return func(x); }); });
    return ret;
}
...
function configureAVDHardware(sdkPath, desc) {
    ...
    return q.nfcall(fs.stat, file)
        .then(function (stats) { return q.nfcall(fs.readFile, file); }, function (err) { return q(''); })
...
function android(sdkPath, apiLevels, abis, acceptLicenses, version, logger) {
    ...
    downloadAndroidUpdates(sdkPath, tools, false, acceptLicenses)
        .then(function () { return setupHardwareAcceleration(sdkPath); })
        .then(function () {
        ...
        .then(function () { return getAVDDescriptors(sdkPath); })
        ...
        .then(function () {
        return q.nfcall(fs.writeFile, path.join(sdkPath, 'available_avds.json'), JSON.stringify(avdDescriptors.map(function (descriptor) { return descriptor.name; })));
    })
        .then(function () { logger.info('android-sdk: Initialization complete'); })
        .done();
}
